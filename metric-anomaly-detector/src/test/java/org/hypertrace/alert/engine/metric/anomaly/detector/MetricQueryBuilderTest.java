...
public class MetricQueryBuilderTest {
  ...
  void testBuildMetricQueryRequest() {
    ...
    LhsExpression lhsExpression = LhsExpression.newBuilder().setAttribute(
        Attribute.newBuilder().setKey("id").setScope("SERVICE").build()).build();
    RhsExpression rhsExpression = RhsExpression.newBuilder().setStringValue("1234").build();
    LeafFilter leafFilter = LeafFilter.newBuilder()
        .setValueOperator(ValueOperator.VALUE_OPERATOR_EQ)
        .setLhsExpression(lhsExpression)
        .setRhsExpression(rhsExpression)
        .build();

    MetricSelection metricSelection = MetricSelection.newBuilder()
        .setMetricAggregationInterval("PT15s")
        .setMetricAggregationFunction(MetricAggregationFunction.METRIC_AGGREGATION_FUNCTION_TYPE_SUM)
        .setFilter(Filter.newBuilder().setLeafFilter(leafFilter).build())
        .setMetricAttribute(Attribute.newBuilder().setKey("duration").setScope("SERVICE").build())
        .build();
    ...
    QueryRequest queryRequest = mqb.buildMetricQueryRequest(metricSelection, 100, 200);
  }
  ...
  void temp() throws URISyntaxException, MalformedURLException {
    ...
    LhsExpression lhsExpression = LhsExpression.newBuilder().setAttribute(
        Attribute.newBuilder().setKey("id").setScope("SERVICE").build()).build();
    RhsExpression rhsExpression = RhsExpression.newBuilder().setStringValue("5efcdda0-2a81-3c19-bf66-2187891bdba1").build();
    LeafFilter leafFilter = LeafFilter.newBuilder()
        .setValueOperator(ValueOperator.VALUE_OPERATOR_EQ)
        .setLhsExpression(lhsExpression)
        .setRhsExpression(rhsExpression)
    ...
    QueryRequest queryRequest = metricQueryBuilder.buildMetricQueryRequest(
        metricSelection,
        System.currentTimeMillis() - Duration.ofHours(2).toMillis(), System.currentTimeMillis());
    ...
    /**
     * Select dateTimeConvert(start_time_millis,?,?,?), SUM(duration_millis)
     * FROM rawServiceView WHERE tenant_id = ? AND ( ( start_time_millis >= ? AND start_time_millis < ? )
     * AND service_id = ? ) GROUP BY dateTimeConvert(start_time_millis,?,?,?) limit 10000
     *
     * Select dateTimeConvert(start_time_millis,?,?,?), SUM(duration_millis) FROM rawServiceView WHERE tenant_id = ?
     * AND ( ( start_time_millis >= ? AND start_time_millis < ? ) AND service_id = ? )
     * GROUP BY dateTimeConvert(start_time_millis,?,?,?) limit 10000 -> "Params{integerParams={},
     * longParams={4=1624970911920, 5=1624978111920}, stringParams={0=1:MILLISECONDS:EPOCH,
     * 1=1:MILLISECONDS:EPOCH, 2=15:SECONDS, 3=default, 6=b89ddf9b-c582-3d93-9006-a4b3024cf5a5,
     * 7=1:MILLISECONDS:EPOCH, 8=1:MILLISECONDS:EPOCH, 9=15:SECONDS}, floatParams={}, doubleParams={}, byteStringParams={}}"
     *
      Select dateTimeConvert(start_time_millis,'1:MILLISECONDS:EPOCH','1:MILLISECONDS:EPOCH','15:SECONDS'),
      SUM(duration_millis) FROM rawServiceView WHERE tenant_id = 'default' AND ( ( start_time_millis >= 1624970911920
      AND start_time_millis < 1624978111920 ) AND service_id = '5efcdda0-2a81-3c19-bf66-2187891bdba1' )
      GROUP BY dateTimeConvert(start_time_millis,'1:MILLISECONDS:EPOCH','1:MILLISECONDS:EPOCH','15:SECONDS') limit 10000
     */
  }
}